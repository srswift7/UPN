...
'
' Copyright (c) 1992-2020 g.dargel <srswift@arcor.de> www.srswift.de
'
...
'  48 Zahlenspeicher (0-47) im EEPROM  (K_num_mem)
'  8 Zeichenkettenspeicher a 16 Byte fuer Input-Statements
'  16 Unterprogrammebenen mit GOSUB und RETURN
'
'  Updates: Updates: 5.0 Initialversion, basiert auf der V3.22
'                        LCD-Display. groesserer Speicher, Sleep ueber LCD-Beleuchtung, Statuszeile
'                        8x4-Tastenfeld, nur offline Programmierbar, Zeichenkettenanzeige
'                    5.08 Kleine Korrekturen
'
'----------------------------------------------------------

$regfile = "m328pdef.dat"                                   ' ATmega328-Deklarationen

' $prog &HFF , &H62 , &HD9 , &HFF                             ' Original 1MHz generated. Take care that the chip supports all fuse bytes.

$prog &HFF , &HE2 , &HD9 , &HFF                             ' 8MHz generated. Take care that the chip supports all fuse bytes.

$crystal = 8000000                                          ' Kein Quarz: 8MHz MHz
...
Declare Sub Anzeige_xy()                                    ' Kombination aus Interpr_xy und Anzeigen
...
Declare Sub Pausexs(zeit As Byte)                           ' zeit * 1/10 Sekunde Pause
...
Declare Sub Display_str(byval Strptr As Byte)               ' Zeichenkette in der Statzuszeile ausgeben
...
Declare Function Read_block(nextblock As Byte) As Byte      ' Einen 12 Byte Block lesen
...
Const K_num_mem = 48                                        ' Anzahl der Zahlenspeicher
Const K_num_prg = 255                                       ' Anzahl der Programmschritte
Const K_num_strg = 128                                      ' Zeichenkettenspeicher 8 * 16 Bytes
...
Dim Str_ptr_ee As Byte                                      ' Pointer auf den Zeichenkettenspeicher

Dim Trans_input As Double                                   ' Umgewandelte Zahl aus dem Eingabestring
Dim Vz_flag As Bit                                          ' Vorzeichen bei Zahlenspeicher
...
Dim Ee_strg(k_num_strg) As Eram Byte                        ' Zeichenkettenspeicher 8*16
...
Dim P_heartbeat As Byte                                     ' Flag zur Anzeigesteuerung im run-Modus
...
Index_key = 0
Lock_statline = 0
Str_ptr_ee = 1                                              ' Index im Zeichenkettenspeicher

' Die EEPROM-Inhalte koennten nach dem Brennen Unsinn enthalten
' Daher haben wir eine Kennung Ee_program_valid = 77
...
' Beim Einschalten steht der Winkelmodus auf "grad"
...
If Ee_program_valid = 77 Then
   Call Display_str(0)                                      ' Programmnamen anzeigen
End If
...
Sub Polling()
  ...
  Else                                                      ' Interaktiv, kein laufendes Programm
      ...
      Else                                                  ' Keine Ziffer sondern ein Kommando
         ...
         ' Ausnahme: Interaktives P_start braucht keine Adresse!
         If X_kommando = P_start Then
            Adr_input_flag = 0
         End If
...
Function Needs_adress(inputkey As Byte) As Byte
   ...
   Select Case Inputkey
      ...
      Case K_grd
        Needs_adress = 1
      Case P_start
        Needs_adress = 1                                    ' Koennte eine Zeichenkette referenzieren
...
Sub Display_status_line()
    ...
    If Lock_statline = 0 Then                               ' Im falle von String fuer Eingabe ignorieren
       Call Clear_t_st()
...
' ========================================================================
' Den String "USB" in die Anzeige schreiben
' Conect
' ========================================================================
Sub Display_con()
      Local I As Byte
...
' ========================================================================
' Die Zeichenkette aus dem angegebenen Speicher in die y-Anzeige schreiben,
' Mit leerzeichen ggf auffuellen
' ========================================================================
Sub Display_str(byval Strptr As Byte)                       ' Adresse des 16 Byte-Strings im EEPROM-Stringspeicher
      Local I As Byte
      Local Si As Byte
      Local Ee_ptr As Byte
      ...
      If Strptr < 8 Then
         ...
         For I = 1 To 16
            Si = 17 - I
            Incr Ee_ptr                                     ' physische Adressen ab 1
            T_st(si) = Ee_strg(ee_ptr)
            If T_st(si) < D_space Then T_st(si) = D_space   ' 0 durch space ersetzen
         Next I
         ...
         Call Anzeigen
      Else
         Call Display_error( "Z")                           ' Illegale Adresse
      End If
...
Sub Interpr_xy()
   ...
   If P_goflag = 0 Then                                     ' Anzeige nur, wenn wir nicht gerade ein Programm lauren lassen
      If P_programming = 0 Then                             ' interaktiver Auto-modus
         ...
         If Lock_statline = 0 Then                          ' nur wenn die Statuszeile nicht anderweitig genutzt ist
              Call Display_status_line()
         End If
...
' ========================================================================
' Anzeigefunktion,  Kombination aus Interpr_xy und Anzeigen
' wir interpretieren Reg in das W_st Register
' dann Schleife ueber das Anzeigefeld,
...
Sub Display_code_line(byval Code_word As Word)
  ...
  If Strlen < 3 Then
     W_st(11) = To_digit(pcode \ 100)                       '   Um Speicher Zu Sparen Lassen Wir Die Decodierung Der Befehle Erst Mal Weg
     Tmpnr = Pcode Mod 100
     W_st(10) = To_digit(tmpnr \ 10)
     W_st(9) = To_digit(pcode Mod 10)
  Else
     Zpos = 11
...
Sub Pausexs(zeit As Byte)
   ...
   If Zeit > 0 Then
      Intrnd = Zeit * 100                                   ' Wir nutzen die globale Variable nach
      Waitms Intrnd
   End If
...
Function Exec_kdo() As Byte
    ...
    Lock_statline = 0                                       ' die Statuszeile ist wieder aktiv
    ...
    ' Vorbereiten der Adressdaten (Zahlenspeicher)
    ...
    Select Case X_kommando
      ...
      Case P_start                                          ' Start / Stop der Programmausfuehrung
           ...
           If P_goflag = 1 Then                             ' Programm soll gerade gestartet werden
              If Rnd_setup = 0 Then                         ' Falls der Zufallszahlengenerator noch nicht initialisiert wurde ...
                 ___rseed = Sleepflag
                 Rnd_setup = 1
              End If
              Sleepflag = 0                                 ' Beleuchtungsuhr zuruecksetzen
           ...
           Else                                             ' Ein laufendes Programm wurde angehalten
              Call Kill_run()                               ' Hintergrundlicht wieder an bei "HALT" oder "END"
              If X_adresse > 0 Then
                 Call Display_str(x_adresse)                ' Ausgabe der zugeordneten Zeichenkette
              End If
           End If
           Call Beepme
      Case K_nop
      ...
      Case P_nop                                            ' tu nix, im Programmspeicher ist da eine 0 oder 254 besser
           Call Beepme
      Case K_pause                                          ' x Sekunde(n) warten, anzeige an
           Call Pausexs(x_adresse)
      End Select
...
' Aus Platzgruenden nur fuer ausgewaehlte Funktionen
' schoen waere es, alle zu nehmen, die eine Asresse brauchen
' ========================================================================
...
'   //          02 - Programmblock
'   //          03 - Zeichenkettenblock maximal 8 Zeichenketten a 16 Zeichen
'   //          04 - Zahlenspeicher in ASCII-Format, 1. Block
'   //          05 - Zahlenspeicher in ASCII-Format, 2. Block
'   //          06 - Zahlenspeicher in ASCII-Format, 3. Block
'   // Byte 2-9 Datenbytes
...
Sub Upload_block()
   ...
   Local Akt_bl As Integer

   Local Ziffer As Byte                                     ' Flag zum zaehlen der Ziffern

   G_uart_error = 0
   Ee_fixflag = S_disp_float                                ' Eingabe- und Anzeigemodus float

   ' Wir lesen 12 Zeichen in den Lesepuffer Zbuffer, 3 * retry wenn noetig

   Akt_bl = &HFF
   I = 0
   While F_nextblock <> Akt_bl And I < 3
      Akt_bl = Read_block(f_nextblock)
      Incr I
   Wend


   If Akt_bl = F_nextblock Then
       ' Sauber gelesen!
       F_blocknr = Zbuffer(1)
       ...
       ' soweit alles OK, was machen wir jetzt mit den gelesenen Daten?
       '   //          01 - Info-Block, wird nicht gespeichert
       '   //          02 - Programmblock
       '   //          03 - Zeichenkettenblock maximal 8 Zeichenketten a 16 Zeichen
       '   //          04 - Zahlenspeicher in ASCII-Format, 1. Block
       ...
       ' Wenn es der Headerblock (0) war , wissen wir jetzt einige Steuerdaten
       If F_blockuse = 1 Or F_blocknr = 0 Then              ' Startadresse lesen und merken
          F_blockptr = Zbuffer(3) + 1                       ' Wir bekommen die logische Startadresse 0, brauchen aber den Index 1, sonst fehlt der erste Befehl
          Str_ptr_ee = 1                                    ' Index im Zeichenkettenspeicher, beim Lesen des Haederblocks setzen wir zurueck
       End If
       ...
       ' Zeichenketten kommen maximal 128 Bytes, Aus dem Datenstrom werden sie einfach als Block in den EEProm kopiert
       ' Die Adressierung macht der Compiler
       ' Dargestellt werden immer 16 Zeichen
       If F_blockuse = 3 Then                               '  Zeichenketten folgen

          If Str_ptr_ee < 113 Then                          ' Maximal 128 Zeichen
             For I = 3 To 10
                Ee_strg(str_ptr_ee) = Zbuffer(i)            ' Im Zeichenkettenspeicher Ablegen
                Incr Str_ptr_ee
             Next I
          Else
             F_blockfolg = &HFF                             ' nicht weiterlesen!
       ...
       If F_blockuse = 4 Then                               '  Zahlenspeicher ( Block 1)
          Call Clear_input()
          Z_inputflag = 0
          P_programming = 0
          I_pt = 1

          ' Das Speichern machen wir ueber Rx und dann mit der normalen STO-Funktion
          X_adresse = Zbuffer(4) + 1                        ' Speicherzelle schon mal merken
          If Zbuffer(5) = 1 Then                            ' Das Vorzeichen
             Vz_flag = 1
          Else
             Vz_flag = 0
          End If
          ...
          For I = 6 To 10
             Pressedkey = Zbuffer(i)                        ' Im Input-Speicher Ablegen
             Ziffer = Digit_input(pressedkey)
             If Ziffer <> 0 Then
                Call Input_number()
             End If
          Next I
       End If
       If F_blockuse = 5 Then                               '  Zahlenspeicher ( Block 2)
          For I = 3 To 10
             Pressedkey = Zbuffer(i)                        ' Im Input-Speicher Ablegen
             Ziffer = Digit_input(pressedkey)
             If Ziffer <> 0 Then
          ...
          Next I
       End If
       If F_blockuse = 6 Then                               '  Zahlenspeicher ( Block 3)
          For I = 3 To 5
             Pressedkey = Zbuffer(i)                        ' Im Input-Speicher Ablegen
             Ziffer = Digit_input(pressedkey)
             If Ziffer <> 0 Then
          ...
          If X_adresse > 0 And X_adresse <= K_num_mem Then
            Ce_mem(x_adresse) = Rx
            Fe_mem(x_adresse) = 1                           ' Cache als modifiziert markieren
          Else
            Call Display_error( "Z")
          End If
       End If
...
' Protokolliges Empfangen von einem Datenblock mit Pruefsumme und Quittung
' Wir bekommen ein Paket von 12 Binär-Bytes
' Parameter: Erwartete Blocknummer
' Return: gelesene Blocknummer
' Im Fehlerfall: FF
' ========================================================================
Function Read_block(nextblock As Byte) As Byte
   Local Summe As Byte
   Local I As Byte
   Local J As Byte
   Local Flag As Byte
   ...
   If Summe = Zbuffer(11) And Nextblock = Zbuffer(1) Then
      ' Quittung senden
      Printbin &H58 ;
      Read_block = Nextblock
   Else
      Printbin &H59 ;
   End If
   Printbin Read_block ;
   Printbin G_uart_error;
...
'   // Byte 0: Blocknummer, = 0
'   // Byte 1: Verwendung   = 01 (Info-Block, wird nicht gespeichert)
'   // Byte 2 Startadresse des Programme im Speicher
